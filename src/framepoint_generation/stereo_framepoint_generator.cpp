...
StereoFramePointGenerator::StereoFramePointGenerator(
    StereoFramePointGeneratorParameters* parameters_)
    : BaseFramePointGenerator(parameters_), _parameters(parameters_) {
  _epipolar_search_offsets_pixel.clear();
  LOG_INFO(std::cerr
           << "StereoFramePointGenerator::StereoFramePointGenerator|constructed"
           << std::endl)
}

void StereoFramePointGenerator::configure() {
  LOG_INFO(std::cerr << "StereoFramePointGenerator::configure|configuring"
                     << std::endl)
  ...
  if (_baseline_meters <= 0) {
    throw std::runtime_error(
        "StereoFramePointGenerator::configure|invalid baseline (m): '" +
        std::to_string(_baseline_meters) +
        "' verify intrinsic camera parameters");
  }
  ...
  _feature_matcher_right.configure(_number_of_rows_image,
                                   _number_of_cols_image);

  // ds configure epipolar search ranges (minimum 0)
  _epipolar_search_offsets_pixel.push_back(0);
  for (int32_t u = 1; u <= _parameters->maximum_epipolar_search_offset_pixels;
       ++u) {
  ...
  // ds info
  LOG_INFO(std::cerr << "StereoFramePointGenerator::configure|baseline (m): "
                     << _baseline_meters << std::endl)
  LOG_INFO(std::cerr << "StereoFramePointGenerator::configure|number of "
                        "epipolar lines considered for stereo matching: "
                     << _epipolar_search_offsets_pixel.size() << std::endl)
  LOG_INFO(std::cerr << "StereoFramePointGenerator::configure|configured"
                     << std::endl)
}
...
StereoFramePointGenerator::~StereoFramePointGenerator() {
  LOG_INFO(std::cerr
           << "StereoFramePointGenerator::~StereoFramePointGenerator|destroying"
           << std::endl)
  _epipolar_search_offsets_pixel.clear();
  LOG_INFO(std::cerr
           << "StereoFramePointGenerator::~StereoFramePointGenerator|destroyed"
           << std::endl)
}

void StereoFramePointGenerator::initialize(Frame* frame_,
                                           const bool& extract_features_) {
  if (!frame_) {
    throw std::runtime_error(
        "StereoFramePointGenerator::initialize|called with empty frame");
  }

  // ds check if a new feature extraction is desired (the frame might already be
  // set up)
  if (extract_features_) {
    // ds detect new features - check if we have information from a previous
    // computation
    if (frame_->previous()) {
      detectKeypoints(frame_->intensityImageLeft(), frame_->keypointsLeft(),
                      frame_->previous()->hasReliablePoseEstimate());
    ...
    computeDescriptors(frame_->intensityImageLeft(), frame_->keypointsLeft(),
                       frame_->descriptorsLeft());
    computeDescriptors(frame_->intensityImageRight(), frame_->keypointsRight(),
                       frame_->descriptorsRight());
    _number_of_detected_keypoints = frame_->keypointsLeft().size();
    LOG_DEBUG(std::cerr
              << "StereoFramePointGenerator::initialize|extracted features L: "
              << frame_->keypointsLeft().size()
              << " R: " << frame_->keypointsRight().size() << std::endl)

    // ds set maximum descriptor distance for triangulation depending on on
    // state
    if (frame_->status() == Frame::Localizing) {
      // ds be conservative while localizing
      _current_maximum_descriptor_distance_triangulation =
          std::min(0.1 * SRRG_PROSLAM_DESCRIPTOR_SIZE_BITS,
                   _parameters->maximum_matching_distance_triangulation);
    } else {
      // ds adjust triangulation distance: few points > narrow window as we
      // cannot permit a relatively high ratio of invalid triangulations
      const real ratio_available_points =
          std::min(static_cast<real>(_number_of_detected_keypoints) /
                       _target_number_of_keypoints,
                   1.0);
      _current_maximum_descriptor_distance_triangulation =
          std::max(ratio_available_points *
                       _parameters->maximum_matching_distance_triangulation,
  ...
  _feature_matcher_left.setFeatures(frame_->keypointsLeft(),
                                    frame_->descriptorsLeft());
  _feature_matcher_right.setFeatures(frame_->keypointsRight(),
                                     frame_->descriptorsRight());
}
...
void StereoFramePointGenerator::compute(Frame* frame_) {
  ...
  if (!frame_) {
    throw std::runtime_error(
        "StereoFramePointGenerator::compute|called with empty frame");
  }
  ...
  if (_parameters->enable_keypoint_binning) {
    for (FramePoint* point : frame_->points()) {
      const Index row_bin = std::rint(static_cast<real>(point->row) /
                                      _parameters->bin_size_pixels);
      const Index col_bin = std::rint(static_cast<real>(point->col) /
                                      _parameters->bin_size_pixels);
      _bin_map_left[row_bin][col_bin] = point;
    }
  }

  // ds prepare for fast stereo matching - we temporally break the
  // IntensityFeature.index_in_vector, which will be restored when pruning
  ...
  if (_parameters->use_matches) {
    ...
    if (_parameters->matching_disable_findhomography) {
      ...
      for (std::vector<cv::DMatch>::const_iterator it = good_matches.begin();
           it != good_matches.end(); ++it) {
        before_left_good_points.push_back(
            frame_->keypointsLeft()[it->queryIdx].pt);
        before_right_good_points.push_back(
            frame_->keypointsRight()[it->trainIdx].pt);
      }
      ...
      if (_parameters->findhomography_method == "RANSAC")
        H = cv::findHomography(
            before_left_good_points, before_right_good_points, cv::RANSAC,
            _parameters->maximum_ransac_reproject, mask,
            _parameters->maximum_iters, _parameters->maximum_confidence);
      else if (_parameters->findhomography_method == "RHO")
        H = cv::findHomography(
            before_left_good_points, before_right_good_points, cv::RHO,
            _parameters->maximum_ransac_reproject, mask,
            _parameters->maximum_iters, _parameters->maximum_confidence);
      else if (_parameters->findhomography_method == "LMEDS")
        H = cv::findHomography(
            before_left_good_points, before_right_good_points, cv::LMEDS,
            _parameters->maximum_ransac_reproject, mask,
            _parameters->maximum_iters, _parameters->maximum_confidence);
      else {
  ...
  for (const int32_t& epipolar_offset : _epipolar_search_offsets_pixel) {
    IntensityFeaturePointerVector& features_left(
        _feature_matcher_left.feature_vector);
    IntensityFeaturePointerVector& features_right(
        _feature_matcher_right.feature_vector);
    ...
    for (uint32_t index_L = 0; index_L < features_left.size(); index_L++) {
      ...
      while (features_left[index_L]->row <
             features_right[index_R]->row + epipolar_offset) {
        index_L++;
      ...
      while (feature_left->row ==
        ...
        const real descriptor_distance =
            cv::norm(feature_left->descriptor,
      ...
      if (descriptor_distance_best <
        ...
        if (_parameters->enable_keypoint_binning) {
          const Index row_bin = std::rint(static_cast<real>(feature_left->row) /
                                          _parameters->bin_size_pixels);
          const Index col_bin = std::rint(static_cast<real>(feature_left->col) /
                                          _parameters->bin_size_pixels);
          ...
          if (_bin_map_left[row_bin][col_bin]) {
            ...
            // ds if the point in the bin is not tracked, we prefer points with
            // maximal disparity (= maximally accurate depth estimate)
            if (!current->previous() &&
        ...
        // ds block further matching NOTE: that we have to put the indices of
        // the sorted vector instead of using IntensityFeature.index_in_vector
        // ds this because the IntensityFeature.index_in_vector gets broken
        // after the sorting and we don't want to spend time on maintaining it
        // for this task
        matched_indices_left.insert(index_L);
        matched_indices_right.insert(index_best_R);
        _feature_matcher_left
            .feature_lattice[feature_left->row][feature_left->col] = nullptr;
        _feature_matcher_right
            .feature_lattice[feature_right->row][feature_right->col] = nullptr;

        // ds reduce search space (this eliminates all structurally conflicting
        // matches)
    ...
    LOG_DEBUG(
        std::cerr << "StereoFramePointGenerator::compute|epipolar offset: "
                  << epipolar_offset
  ...
  LOG_DEBUG(
      std::cerr
      << "StereoFramePointGenerator::compute|number of new stereo points: "
      << number_of_new_points << "/" << _number_of_detected_keypoints
      << std::endl)
  ...
  if (_parameters->enable_keypoint_binning) {
    ...
    LOG_DEBUG(std::cerr << "StereoFramePointGenerator::compute|number of new "
                           "stereo points binned: "
                        << number_of_points_binned - number_of_points_tracked
                        << std::endl)
  } else {
    // ds add all points to frame
    framepoints.insert(framepoints.end(), framepoints_new.begin(),
                       framepoints_new.end());
  }
  CHRONOMETER_STOP(point_triangulation)
}

void StereoFramePointGenerator::track(
    Frame* frame_, Frame* frame_previous_,
    const TransformMatrix3D& camera_left_previous_in_current_,
    FramePointPointerVector& lost_points_, const bool track_by_appearance_) {
  if (!frame_ || !frame_previous_) {
    throw std::runtime_error(
        "StereoFramePointGenerator::track|called with invalid frames");
  }
  ...
  // ds tracked and triangulated features (to not consider them in the
  // exhaustive stereo matching)
  std::set<uint32_t> matched_indices_left;
  std::set<uint32_t> matched_indices_right;
  ...
  for (FramePoint* point_previous : framepoints_previous) {
    ...
    const Vector3 point_in_camera_left_prediction(
        camera_left_previous_in_current_ *
        point_previous->cameraCoordinatesLeft());

    // ds project the point into the current left image plane
    const Vector3 point_in_image_left(camera_calibration_matrix *
                                      point_in_camera_left_prediction);
    const int32_t col_projection_left =
        point_in_image_left.x() / point_in_image_left.z();
    const int32_t row_projection_left =
        point_in_image_left.y() / point_in_image_left.z();

    // ds skip point if not in image plane
    if (col_projection_left < 0 ||
        col_projection_left > _number_of_cols_image ||
        row_projection_left < 0 ||
    ...
    int32_t row_start_point =
        std::max(row_projection_left - _projection_tracking_distance_pixels, 0);
    int32_t row_end_point =
        std::min(row_projection_left + _projection_tracking_distance_pixels + 1,
                 _number_of_rows_image);
    int32_t col_start_point =
        std::max(col_projection_left - _projection_tracking_distance_pixels, 0);
    int32_t col_end_point =
        std::min(col_projection_left + _projection_tracking_distance_pixels + 1,
                 _number_of_cols_image);

    // ds find the best match for the previous left feature (i.e. track it)
    IntensityFeature* feature_left =
        _feature_matcher_left.getMatchingFeatureInRectangularRegion(
            row_projection_left, col_projection_left,
            point_previous->descriptorLeft(), row_start_point, row_end_point,
            col_start_point, col_end_point,
            _maximum_descriptor_distance_tracking, track_by_appearance_,
            descriptor_distance_best);

    // ds if we found a match
    if (feature_left) {
      // ds compute projection offset (i.e. prediction error > optical flow)
      const cv::Point2f projection_error(
          col_projection_left - feature_left->keypoint.pt.x,
          row_projection_left - feature_left->keypoint.pt.y);

      // ds project point into the right image - correcting by the prediction
      // error
      const Vector3 point_in_image_right(point_in_image_left + _baseline);
      const int32_t col_projection_right_corrected =
          point_in_image_right.x() / point_in_image_right.z() -
          projection_error.x;
      ...
      // ds skip point if not in image plane
      if (col_projection_right_corrected < 0 ||
          col_projection_right_corrected > _number_of_cols_image ||
          row_projection_right_corrected < 0 ||
          row_projection_right_corrected > _number_of_rows_image) {
        continue;
      }

      // ds TRIANGULATION: obtain matching feature in right image (if any)
      // ds we reduce the vertical matching space to the epipolar range - we
      // search only to the left of the measure left camera coordinate
      const int32_t epipolar_offset_previous =
          std::fabs(point_previous->epipolarOffset());
      row_start_point = std::max(
          row_projection_right_corrected - epipolar_offset_previous, 0);
      row_end_point = std::min(
          row_projection_right_corrected + epipolar_offset_previous + 1,
          _number_of_rows_image);
      col_start_point = std::max(
          col_projection_right_corrected - _projection_tracking_distance_pixels,
          0);
      col_end_point = std::min(col_projection_right_corrected +
                                   _projection_tracking_distance_pixels + 1,
                               feature_left->col);

      // ds we might increase the matching tolerance
      // (maximum_matching_distance_triangulation) since we have a strong prior
      // on location
      IntensityFeature* feature_right =
          _feature_matcher_right.getMatchingFeatureInRectangularRegion(
              row_projection_right_corrected, col_projection_right_corrected,
              feature_left->descriptor, row_start_point, row_end_point,
              col_start_point, col_end_point,
              _current_maximum_descriptor_distance_triangulation, true,
              descriptor_distance_best);
      ...
      if (feature_right) {
        ...
        if (cv::norm(feature_right->descriptor,
                     point_previous->descriptorRight(),
                     SRRG_PROSLAM_DESCRIPTOR_NORM) >
            _maximum_descriptor_distance_tracking) {
          continue;
        }

        // ds remove remaining matches in parallax between left and right point
        // in the right image
        for (int32_t col = feature_right->col + 1; col < feature_left->col;
             ++col) {
          if (_feature_matcher_right.feature_lattice[feature_right->row][col]) {
            matched_indices_right.insert(
                _feature_matcher_right.feature_lattice[feature_right->row][col]
                    ->index_in_vector);
            _feature_matcher_right.feature_lattice[feature_right->row][col] =
                nullptr;
          }
        ...
        framepoint->setProjectionEstimateLeft(
            cv::Point2f(col_projection_left, row_projection_left));
        framepoint->setProjectionEstimateRight(
            cv::Point2f(point_in_image_right.x() / point_in_image_right.z(),
                        point_in_image_right.y() / point_in_image_right.z()));
        framepoint->setProjectionEstimateRightCorrected(cv::Point2f(
            col_projection_right_corrected, row_projection_right_corrected));
  ...
  frame_previous_->setAverageDescriptorDistanceTracking(
      accumulated_descriptor_distance / number_of_tracked_points);
  ...
  LOG_DEBUG(
      std::cerr
      << "StereoFramePointGenerator::track|tracked and triangulated points: "
      << number_of_tracked_points << "/" << framepoints_previous.size()
      << " (landmarks: " << _number_of_tracked_landmarks << ")" << std::endl)
  LOG_DEBUG(std::cerr << "StereoFramePointGenerator::track|lost points: "
                      << number_of_points_lost << "/"
                      << framepoints_previous.size() << std::endl)
}

void StereoFramePointGenerator::recoverPoints(
    Frame* current_frame_, const FramePointPointerVector& lost_points_) const {
  // ds precompute transforms
  const TransformMatrix3D world_to_camera_left =
      current_frame_->worldToCameraLeft();
  ...
  current_frame_->points().resize(number_of_tracked_points +
                                  lost_points_.size());
  for (FramePoint* point_previous : lost_points_) {
    ...
    if (point_previous->landmark()) {
      ...
      point_in_camera_homogeneous =
          world_to_camera_left * point_previous->landmark()->coordinates();
    } else {
      // ds get point in camera frame based on point coordinates
      point_in_camera_homogeneous =
          world_to_camera_left * point_previous->worldCoordinates();
    }

    // ds obtain point projection on camera image plane
    PointCoordinates point_in_image_left =
        camera_calibration_matrix * point_in_camera_homogeneous;
    PointCoordinates point_in_image_right =
        point_in_image_left + baseline_homogeneous;

    // ds skip invalid depths
    if (point_in_image_left.z() < _parameters->minimum_depth_meters ||
        point_in_image_left.z() > _parameters->maximum_depth_meters ||
        point_in_image_right.z() < _parameters->minimum_depth_meters ||
        point_in_image_right.z() > _parameters->maximum_depth_meters) {
      continue;
    }
    ...
    const cv::Point2f projection_left(std::rint(point_in_image_left.x()),
                                      std::rint(point_in_image_left.y()));
    const cv::Point2f projection_right(std::rint(point_in_image_right.x()),
                                       std::rint(point_in_image_right.y()));

    // ds check if projection range is insufficient for recovery
    const float regional_border_center =
        5 * point_previous->keypointLeft().size;
    if (projection_left.x < regional_border_center + 1 ||
        projection_left.x >
            _camera_left->numberOfImageCols() - regional_border_center - 1 ||
        projection_right.x < regional_border_center + 1 ||
        projection_right.x >
            _camera_left->numberOfImageCols() - regional_border_center - 1 ||
        projection_left.y < regional_border_center + 1 ||
        projection_left.y >
            _camera_left->numberOfImageRows() - regional_border_center - 1 ||
        projection_right.y < regional_border_center + 1 ||
        projection_right.y >
            _camera_left->numberOfImageRows() - regional_border_center - 1) {
      continue;
    }
    ...
    const cv::Point2f offset_keypoint_half(regional_border_center,
                                           regional_border_center);
    const float regional_full_height =
        regional_border_center + regional_border_center + 1;

    // ds left search regions
    const cv::Point2f corner_left(projection_left - offset_keypoint_half);
    const cv::Rect_<float> region_of_interest_left(corner_left.x, corner_left.y,
    ...
    _descriptor_extractor->compute(roi_left, keypoint_buffer_left,
                                   descriptor_left);
    ...
    if (cv::norm(point_previous->descriptorLeft(), descriptor_left,
                 SRRG_PROSLAM_DESCRIPTOR_NORM) >
        _maximum_descriptor_distance_tracking) {
      continue;
    }
    ...
    const cv::Rect_<float> region_of_interest_right(
    ...
    _descriptor_extractor->compute(roi_right, keypoint_buffer_right,
                                   descriptor_right);
    ...
    // ds if descriptor distance is to high
    if (cv::norm(point_previous->descriptorRight(), descriptor_right,
                 SRRG_PROSLAM_DESCRIPTOR_NORM) >
        _maximum_descriptor_distance_tracking) {
      continue;
    }
    ...
    const real descriptor_distance_triangulation = cv::norm(
        descriptor_left, descriptor_right, SRRG_PROSLAM_DESCRIPTOR_NORM);
    if (descriptor_distance_triangulation >
        _current_maximum_descriptor_distance_triangulation) {
      continue;
    }
    ...
    IntensityFeature* feature_left =
        new IntensityFeature(keypoint_buffer_left[0], descriptor_left, 0);
    IntensityFeature* feature_right =
        new IntensityFeature(keypoint_buffer_right[0], descriptor_right, 0);

    // ds allocate a new point connected to the previous one
    FramePoint* current_point = current_frame_->createFramepoint(
        feature_left, feature_right, descriptor_distance_triangulation,
        getPointInLeftCamera(keypoint_buffer_left[0].pt,
                             keypoint_buffer_right[0].pt),
        point_previous);
  ...
  LOG_DEBUG(
      std::cerr << "StereoFramePointGenerator::recoverPoints|recovered points: "
                << current_frame_->points().size() - number_of_tracked_points
                << "/" << lost_points_.size() << std::endl)
}

const PointCoordinates StereoFramePointGenerator::getPointInLeftCamera(
    const cv::Point2f& image_coordinates_left_,
    const cv::Point2f& image_coordinates_right_) const {
  ...
  position_in_left_camera.z() =
      _b_x / (image_coordinates_right_.x - image_coordinates_left_.x);

  // ds compute x in camera (regular)
  position_in_left_camera.x() = 1 / _f_x * (image_coordinates_left_.x - _c_x) *
                                position_in_left_camera.z();

  // ds average in case we have an epipolar offset in v
  position_in_left_camera.y() =
      1 / _f_y *
      ((image_coordinates_left_.y + image_coordinates_right_.y) / 2.0 - _c_y) *
      position_in_left_camera.z();
